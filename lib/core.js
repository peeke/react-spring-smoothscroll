...
export function SmooshScroll({ transition }) {
  if (!usedOnce) throw new Error('Multiple instances of SmoothScroll found')
  usedOnce = false
  ...
  transition.setImmediate(document.scrollingElement.scrollTop)
  ...
  return () => { target.removeEventListener('wheel', handleWheel) }
    ...
    transition.set(clamp({
      ...
      input: transition.target + normalizeWheelDelta(e)
    }))
